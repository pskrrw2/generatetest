...
(function () {
    const phoneMaskList = document.querySelectorAll('.phone-mask'),
        ...
        timepickerList = document.querySelectorAll('.time-picker'),
    ...
    // Flat Picker Time
    if (timepickerList) {
        timepickerList.forEach(function (timepicker) {
    ...
    if (formCheckInputPayment) {
        formCheckInputPayment.forEach(function (paymentInput) {
            paymentInput.addEventListener('change', function (e) {
                ...
                const formSection = e.currentTarget.closest('.col-12');  // Get the closest form section

                if (paymentInputValue === 'PartialBooking') {
                    formSection.querySelector('#form-partial-suite-options').classList.remove('d-none');
                } else {
                    formSection.querySelector('#form-partial-suite-options').classList.add('d-none');
                }
            });
        });
    }
